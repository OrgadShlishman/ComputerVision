...
def get_xception_based_model() -> nn.Module:
    ...
    # Building an Xception pre-trained backbone and hold it as `custom_network`
    custom_network = build_xception_backbone(True)
    # Overriding `custom_network`'s fc attribute with the binary classification head stated in the exercise.
    custom_network.fc = MLP_head()
