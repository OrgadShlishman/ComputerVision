...
class Trainer:
    ...
    def train_one_epoch(self) -> tuple[float, float]:
        ...
        for batch_idx, (inputs, targets) in enumerate(train_dataloader):
            ...
            # Computing a forward pass: computing predictions
            predictions = self.model(inputs)
            # Computing the loss w.r.t to the criterion
            ...
            correct_labeled_samples += (predictions.argmax(1) == targets).type(torch.float).sum().item()

            nof_batches = batch_idx + 1
            nof_samples += len(targets)

            avg_loss = total_loss / nof_batches
    ...
    def evaluate_model_on_dataloader(
        ...
        for batch_idx, (inputs, targets) in enumerate(dataloader):
            ...
            targets = targets.to(device)
            with torch.no_grad():
                # Computing a forward pass under a torch.no grad() context manager
                predications = self.model(inputs)
                # Computing the loss w.r.t to the criterion
                total_loss += self.criterion(predications, targets).item()
                correct_labeled_samples += (predications.argmax(1) == targets).type(torch.float).sum().item()
                # Updating the average loss and accuracy
                nof_batches = batch_idx + 1
