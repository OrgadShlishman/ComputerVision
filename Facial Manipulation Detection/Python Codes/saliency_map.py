...
def compute_gradient_saliency_maps(samples: torch.tensor,
    ...
    model.eval()
    # Set requires_grad for the samples
    samples.to(device)
    ...
    scores = predictions.gather(1, true_labels.view(-1, 1)).squeeze()
    ...
    # Pick the maximum value from channels (dim = 1) on each pixel.
    saliency_maps, _ = torch.max(abs_gradients, dim=1)
