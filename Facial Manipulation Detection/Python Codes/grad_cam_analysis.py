...
def get_grad_cam_visualization(test_dataset: torch.utils.data.Dataset,
    ...
    # Samples a single image from the dataset
    batch_size = 1
    data_samples = DataLoader(test_dataset, batch_size, shuffle=True)
    (input_image, label) = next(iter(data_samples))

    # Computes a Grad-CAM for that image for the target layer: model.conv3
    cam = GradCAM(model=model, target_layers=[model.conv3], use_cuda=0)
    input_image = input_image.to(device)
    target = label.to(device)
    grayscale_cam = cam(input_tensor=input_image, targets=[ClassifierOutputTarget(target.item())])
    grayscale_cam = grayscale_cam[0, :]

    # Creates an RGB image from the sample
    rgb_img = np.array(input_image[0].cpu().permute(1, 2, 0), dtype=np.float32)
    rgb_img = (rgb_img - rgb_img.min())/(rgb_img.max() - rgb_img.min())

    # return the visualization (see code snippet) and the true label
